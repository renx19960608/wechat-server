...
public class AdminTokenInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if("OPTIONS".equals(request.getMethod().toUpperCase())){
			...
			response.setHeader("Access-Control-Allow-Headers", "admin_token");
			response.setHeader("Access-Control-Allow-Methods", "GET,POST");
			return true;
		}
		// 获取token数据
		String token = request.getHeader(CommonConstant.ADMIN_REQ_TOKEN);
		if (null == token || "".equals(token)) {
			token = request.getParameter(CommonConstant.ADMIN_REQ_TOKEN);
		}
		...
		String tokenRedisKey = CommonConstant.TOKEN_REDIS_KEY_PREFIX+token;
